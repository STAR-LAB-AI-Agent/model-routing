...
- [现场验收指南](docs/acceptance-guide.md)、[接口说明](docs/interface.md)、[设计说明](docs/design.md)
- [实验报告源码](docs/experiment-report.md) 与 `deliverables/实验报告.pdf`
- `deliverables/demo.mp4`：真实 API 操作视频，内嵌中文字幕
- `examples/` 十条验收案例、`tests/` 测试、`artifacts/` 实测证据与截图
- `skills/` 完整 Skill + Script，`requirements*.txt` 依赖锁定，`licenses/` 许可文本

报告身份信息在 `docs/student-info.json` 填写后，用 `scripts/build_report.py` 重新生成 PDF（需 requirements-artifacts.txt；默认 Windows 宋体）。GitHub 提交用项目文件夹或同级完整提交包；不要上传 runtime 或 .venv。已配置 GitHub Actions Windows/Linux、Python 3.11/3.12 测试矩阵；远端 CI 需推送后才会执行。

视频重录：安装 agent-browser、Chrome 与 ffmpeg，启动服务，执行 `scripts/record-demo.ps1`。它操作页面并真实调用 API，默认输出 artifacts/raw/live-recording.mp4；校验一分钟以内后替换 deliverables/demo.mp4。录屏不会输入或显示密钥。

## 开源与边界

| 项目 | 版本 | 许可证、实际用途 |
|---|---|---|
| [LiteLLM](https://github.com/BerriAI/litellm) | 1.101.0 | SDK MIT，completion 统一真实模型接口 |
| [nanobot](https://github.com/HKUDS/nanobot) | 0.3.5 | MIT，Skill 发现与 ExecTool 调用 |
| [pytest](https://github.com/pytest-dev/pytest) | 8.4.2 | MIT，自动测试 |
| ReportLab | 4.4.9 | BSD，生成报告 PDF |

项目自有代码为 MIT，许可原文在 licenses。规则可能误判隐含难度；小样例只验证功能，不证明普遍模型质量。费用估算不含 nanobot 编排层；模型服务及 nanobot 自身日志不受本项目审计规则约束。本地服务用于单用户课程验收，未实现公网多用户认证、队列或分布式缓存。
